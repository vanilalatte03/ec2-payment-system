...
- 해당 브랜치 구현 과정에서 사용자가 Codex에 보낸 구현 관련 프롬프트를 현재 세션/대화 기록에서 확인하고 1~3개 bullet로 요약한다.
- 현재 세션만으로 확인이 어려우면 접근 가능한 Codex 세션 로그나 저장된 컨텍스트를 확인하되, 권한이 없거나 기록을 찾을 수 없으면 확인 불가로 명확히 적는다.
