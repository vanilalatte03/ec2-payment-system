...
- 검증 통과 시 단일 트랜잭션으로 주문 완료, 결제 완료, 예약 포인트 사용 확정, 포인트 잔액 갱신을 처리합니다. 적립 예정 포인트가 있으면 PG 실결제 금액의 1%를 `EARN` 원장으로 적립합니다.
- 검증 실패 또는 금액 불일치가 외부 결제 성공 후 발견되는 경우, 보상 취소와 주문/재고 복구는 웹훅 및 결제 실패 보상 흐름에서 처리합니다. 포인트 도메인은 예약 포인트 복구가 필요할 때 `USE_CANCEL` 원장 처리를 제공합니다.
