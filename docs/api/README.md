...
공통 응답 wrapper의 상세 정의, 인증 방식, 공통 Enum, 공통 에러 코드는 [common.md](./common.md)에 정의합니다. 각 도메인 문서에는 해당 API의 요청/응답 데이터, 발생 가능한 에러 코드, 공통 wrapper 사용 안내를 적습니다.
...
| [payments.md](./payments.md) | PortOne 브라우저 설정 조회, 결제 확정, 포인트 전액 결제 확정 |
...
| 결제 | PortOne 브라우저 설정 조회 | GET | `/api/payments/portone-config` | 불필요 |
...
- 장바구니는 주문 생성 시점에 삭제하지 않고, 결제 완료 시 주문 상품에 해당하는 장바구니 상품만 삭제합니다.
