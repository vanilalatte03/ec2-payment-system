...
- 결제 확정이 성공하면 주문에 포함된 장바구니 상품만 삭제합니다. 주문하지 않은 다른 장바구니 상품은 그대로 유지됩니다.
...
| 필드 | 타입     | 설명 |
| --- |--------| --- |
| `cartId` | Long   | 장바구니 ID. 장바구니가 없으면 `null` |
...
| `items[].status` | String | 현재 상품 상태. `ON_SALE`, `SOLD_OUT`, `DISCONTINUED` |
